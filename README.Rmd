...
In order to perform random-walk based methods (RWS and RWR), we need to transform the Hi-C contact count matrix into a transition probability matrix (TPM). Here we apply Knight-Ruiz (KR) algorithm to turn a contact matrix (symmetric and all elements being non-negative) into a symmetric TPM. 
